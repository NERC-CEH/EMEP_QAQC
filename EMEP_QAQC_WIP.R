...
TEST_OUTER_FNAME = 'EMEP4UK_emep-ctm-rv4.36_wrf4.2.2_UKSCAPE_BASE_trend2019_emiss2019_EU_2019_fullrun.nc'
TEST_INNER_FNAME = 'EMEP4UK_emep-ctm-rv4.36_wrf4.2.2_UKSCAPE_BASE_trend2019_emiss2019_UK_3Km_2019_fullrun.nc'
REF_OUTER_FNAME = 'EMEP4UK_emep-ctm-rv4.36_wrf4.2.2_ROWE_BASE_trend2019_emiss2019_EU_2019_fullrun.nc'
REF_INNER_FNAME = 'EMEP4UK_emep-ctm-rv4.36_wrf4.2.2_ROWE_BASE_trend2019_emiss2019_UK_3Km_2019_fullrun.nc'

EMEP_BUDGET_FNAME = c(TEST_INNER_FNAME, REF_INNER_FNAME) # test fname MUST be first in the vector
BUDGET_MASK_FNAME = 'Massimo/EMEP_GRID_01x01DEG_GB.shp'

TEST_OBS_FNAME = 'NO2_O3_M25_hour.nc' 

PALETTE_DIR = '.' #where ncl color palettes are stored
...
MBS_TABLE_FNAME = 'MBS_diff'                          # emission comparison between test and ref runs in MassBudgetSummary.txt
...
AUTO_NETWORK = c('aurn')       # the automatic monitoring network used for observation data
                               ...
                               # - the csv files need to have columns: 'code', 'date' (which is hour starting) and poll names as in OBSERVED_POLLS
                               #   below

OBSERVED_POLLS = c('no', 'no2', 'o3',
                   'ox', 'so2', 'pm10', 'pm2.5')      # tested pollutants -if any other compounds added
...
MODSTATS_STATS = c('n', 'FAC2', 'MB',
                   'NMB', 'RMSE', 'r')                #choose stats for model evaluation
...
if (CALCULATE_BUDGET == T | PLOT_DSC == T) {
  ...
  if (length(EMEP_BUDGET_FNAME) == 2) {
    budget_df2 = budget_df %>%
      mutate(run2 = if_else(str_detect(run, 'UKSCAPE'), 'test', 'ref')) %>% 
      #mutate(run2 = if_else(run == EMEP_BUDGET_FNAME[1], 'test', 'ref')) %>% 
  ...
  if (PLOT_DSC == T) {
    ...
    dsc_plots = plot_DSC(emep_budget2[[1]], VAR_PARAMS_LIST, pretty_lab = T)
...
if (PLOT_COMPARISON_MAPS == T) {
  ...
  comp_plots_list = future_map(map_vars, ~calculate_emep_diff(var = .x, run_labels = RUN_LABELS,
                              ...
                              inner_test_fname = NA, inner_ref_fname = NA, emep_crs = EMEP_CRS)) %>% 
...
foo = load_emep_data(TEST_OUTER_FNAME, EMEP_CRS)
...
if(COLLATE_MOBS == T) {
  ...
  #test slice
  emep_slice = load_emep_data(obs_fname, emep_crs = EMEP_CRS, vars = 'SURF_ug_NO2')[[1]] %>% 
    slice('time', 1) %>% 
    st_as_stars()
...
#     compact()
#   
#   #create a lookup table linking poll names with var names in EMEP output
#   mobs_polls = names(mobs_polls_list) %>% 
#     set_names(flatten_chr(mobs_polls_list)) %>% 
#     .[OBSERVED_POLLS] %>% 
#     na.omit()
#   
#   #get modelled year for filtering auto monitoring sites
#   mod_dates = get_emep_floordate(obs_fname)
#   mod_year = unique(year(mod_dates$date))
#   
#   if (!file_exists(AUTO_NETWORK)) {
#     auto_sites = get_auto_meta(pollutant = OBSERVED_POLLS, year = mod_year) %>% 
#       mutate(year = mod_year, .after = poll_info)
#     auto_sites_geo = auto_sites %>% 
#       st_as_sf(coords = c('longitude', 'latitude'), crs = 4326)
#   } else {
#     auto_sites_geo = read_csv(AUTO_NETWORK)
#   }
#   
#   
#   #get a 2D slice of the EMEP output to check if all sites are in the modelled domain
#   # - will decrease run time considerably if not whole domain modelled
#   emep_slice = load_emep_data(obs_fname, emep_crs = EMEP_CRS, vars = mobs_polls[2])[[1]] %>% 
#     slice(time, 1) %>% 
#     st_as_stars() 
#   
#   sites_in = get_sites_in_domain(auto_sites_geo, emep_slice)
#   
#   auto_sites_geo = auto_sites_geo %>% 
#     filter(code %in% sites_in)
#   
#   #write meta data to files - rds for further use and csv for easier human check
#   write_rds(auto_sites_geo, path(data_pth_out, '00Auto_sites_used.rds'))
#   auto_sites_geo %>% 
#     st_drop_geometry() %>% 
#     unnest(poll_info) %>% 
#     write_csv(path(data_pth_out, '00Auto_sites_used.csv'), na ='')
#   
#   sites_iter = auto_sites_geo %>% 
#     split(.$code)
#   
#   #collate modelled and observed data to a list
#   future::plan(multicore)
#   
#   mobs_list = future_map(sites_iter[1:2], safely(collate_obs_mod3), emep_pth = obs_fname,
#                            poll_lookup = mobs_polls,.options = furrr_options(seed = T)) %>%
#     transpose()
#   
#   # and filter successful outcomes
#   is_ok <- mobs_list$error %>%
#     map_lgl(is_null)
#   
#   mobs_list = mobs_list$result[is_ok]
#   
#   if (SAVE_MOBS_RDS == T) {
#     
#     mobs_data_pths = path(data_pth_out, paste0(names(mobs_list), '_mod_obs'), ext = 'rds')
#     future_walk2(mobs_list, mobs_data_pths, write_rds, .options = furrr_options(seed = T))
#   }
#   
#   if (PLOT_MOBS_PDF == T) {
#     future_walk(mobs_list, site_time_series_pdf3, auto_sites_geo, out_dir = dir_create(path(plots_pth_out, 'Individual_sites')),
#                 ppp = PPP, run_title_info = run_info)
#   }
#   
# }
...
# 
# #plot scatter plots of annual means (like for like - missing observations are dropped from modelled data too)
# mobs_plots = plot_annual_scatter(mobs_list = mobs_list, site_meta_df = auto_sites_geo2, poll_name_lookup = mobs_polls2)
# mobs_plots = mobs_plots[order(match(mobs_plots, OBSERVED_POLLS))]
# 
# #get EMEP run fname for title on pdf plots
# run_info = format_maps_page_title(inner_test_pth = obs_fname, run_labels = 'EMEP run')
# 
# page_titles = rep(run_info, length(mobs_plots) %/% PPP + 1)
# export = marrangeGrob(grobs = mobs_plots, nrow = PPP , ncol = 1, top = substitute(page_titles[g]))
# ggsave(filename = path(plots_pth_out, 'Anual_Mean_Scatter_all_sites.pdf'), export, paper = 'a4', height = 10, width = 7)
# 
# 
# future_walk(merged, site_time_series_pdf3, auto_sites_geo, ppp = PPP, run_title_info = run_info)
# future_pwalk(list(site_code = names(merged),
#                   merged_df = merged), 
#              site_time_series_pdf3, auto_sites_meta_df = auto_sites_geo2, run_title_info = run_info)
...
a_meta = importMeta('aqe', all = T) %>% 
  filter(code %in% c('BN1', 'BRE01', 'WIL4')) %>% 
  distinct(code, .keep_all = T) %>% 
  select(code:longitude) %>% 
  mutate(site_type_grp = recode_site_type(site_type), .after = site_type) 
  

baz = map(c('BN1', 'BRE01', 'WIL4'), openair::importAQE, year = 2018)

obs_pths = str_c(c('BN1', 'BRE01', 'WIL4'), '_obs.csv')
walk2(baz, obs_pths, write_csv)

a_meta2 = a_meta %>% 
  mutate(network = obs_pths) %>% 
  write_rds('custom_meta.rds')
...
if (COMPARE_EMISSIONS == T) {
  ...
  emiss_list = map(c(TEST_OUTER_FNAME, TEST_INNER_FNAME), compare_inv_mod_emissions)
...
foo = load_emep_data(TEST_INNER_FNAME, EMEP_CRS) %>% 
  map(st_as_stars)
foo1 = st_as_stars(foo[[1]])

# file summary-------------------------------------------------------------

foo = map2(c(TEST_OUTER_FNAME, TEST_INNER_FNAME), 
           c(REF_OUTER_FNAME, REF_INNER_FNAME), 
           compare_run_emissions)

foo = read_rds('d_content.rds') %>% 
...
f1 = 'config_EMEP_4.36_ROWE_BASE.nml'
f2 = 'config_EMEP_4.36_UKSCAPE_BASE.nml'



# test --------------------------------------------------------------------

MBS_list = read_rds('MBS_list_test.rds')
