...
`r log_info('Model vs Inventory emission differences exceeding the {EMISS_DIFF_THRESHOLD}% threshold for {outer_domain} domain printed in report')`
```
...
`r log_info('Test vs reference emission differences exceeding the {EMISS_DIFF_THRESHOLD}% threshold for {outer_domain} domain printed in report')`
```
...
`r log_info('Model vs Inventory emission differences exceeding the {EMISS_DIFF_THRESHOLD}% threshold for {inner_domain} domain printed in report')`
```
...
`r log_info('Test vs reference emission differences exceeding the threshold for {inner_domain} domain printed in report')`
```
...
if (!is.na(BUDGET_MASK_FNAME)) {
  ...
  emep_budget2 = tryCatch(
    error = function(cnd) {
      emep_budget %>%
        map(~st_as_stars(.x, curvilinear = NULL))
  ...
  emep_budget2 = emep_budget %>%
    map(st_as_stars, curvilinear = NULL)
...
``` {asis, echo = !is.na(BUDGET_MASK_FNAME) && !is.null(v_tbl2), eval = COMPARE_BUDGET_MSC}
...
`r v_tbl2`
`r log_info('Variables in Test vs Reference budget check printed in a table in the report')`
```
...
`r budget_exceed_tbl2`
`r log_info('Variables in Test vs Reference budget check exceeding the {BUDGET_DIFF_THRESHOLD}% threshold printed in a table in the report')`
```
...
`r log_info('A list of vars for which comparison maps have been created printed in a table in the report')`
...
a_means = future_map_dfr(mobs_list, summarise_mobs, avg_time = 'year', data_thresh = MOBS_THRESHOLD,
                         .options = furrr_options(seed = T)) %>%
  left_join(st_drop_geometry(dplyr::select(sites_geo, code, site_type_grp)))
log_info('Annual means calculated')

#temporary workaround for modStats bug
if ('r' %in% MODSTATS_STATS) {
  MODSTATS_STATS2 = setdiff(MODSTATS_STATS2, 'r')
  
  mobs_stats_all = modStats(a_means, statistic = MODSTATS_STATS2, type = c('pollutant')) %>%
    mutate(site_type_grp = 'All',
           pollutant = as.character(pollutant))
    
  msa2_poll = mobs_stats_all %>%
    filter(n > 2) %>% 
    pull(pollutant)
  
  msa2 = modStats(filter(a_means, pollutant %in% msa2_poll), statistic = 'r', type = 'pollutant') %>% 
    mutate(pollutant = as.character(pollutant))
  
  mobs_stats_all = left_join(mobs_stats_all, msa2, by = 'pollutant') %>% 
    relocate(site_type_grp, .after = last_col())
    
  
  mobs_stats = modStats(a_means, statistic = MODSTATS_STATS2, type = c('pollutant', 'site_type_grp')) %>%
    mutate(pollutant = as.character(pollutant),
           site_type_grp = as.character(site_type_grp))
  
  ms2_poll = mobs_stats %>%
    filter(n > 2) %>% 
    select(pollutant, site_type_grp)
  
  ms2 = modStats(semi_join(a_means, ms2_poll, by = c('pollutant', 'site_type_grp')),
                 statistic = 'r', type = c('pollutant', 'site_type_grp')) %>%
    mutate(pollutant = as.character(pollutant),
           site_type_grp = as.character(site_type_grp))
  
  mobs_stats = left_join(mobs_stats, ms2, by = c('pollutant', 'site_type_grp')) %>% 
    relocate(site_type_grp, .after = last_col())
} else {
...
log_info('MOBS statistics calculated')
```
...
mobs_annual_scatter_plots = plot_annual_scatter(mobs_list = mobs_list, site_meta_df = sites_geo, poll_name_lookup = mobs_polls)
mobs_annual_scatter_plots = mobs_annual_scatter_plots[order(match(mobs_annual_scatter_plots, mobs_polls))]
...
page_titles = rep(run_info, length(mobs_annual_scatter_plots) %/% PPP + 1)
export = marrangeGrob(grobs = mobs_annual_scatter_plots, nrow = PPP , ncol = 1, top = substitute(page_titles[g]))
...
ref_a_means = future_map_dfr(ref_mobs_list, summarise_mobs, avg_time = 'year', data_thresh = MOBS_THRESHOLD,
                             .options = furrr_options(seed = T)) %>%
...
`r mobs_vartbl`
`r log_info('A link up table between OBS and EMEP var names printed in the report')`
...
`r log_info('Table of model evaluation stats for the test run printed in the report')`
```
...
`r log_info('Table of model evaluation stats for the test and reference runs printed in the report')`
```
...
#check if all requested pollutants are available in the data, print those missing
missing_polls = setdiff(OBSERVED_POLLS, pull(distinct(report_mobs_list[[1]], pollutant)))
if (length(missing_polls) == 0) {
  log_warn('Data for the requested pollutant {missing_polls} not available. The pollutant is ignored')
}

OBSERVED_POLLS = setdiff(OBSERVED_POLLS, missing_polls)
...
for (i in seq_along(names(report_mobs_list))) {
  ...
  for (j in seq_along(OBSERVED_POLLS)) {
    p_list[[j]] = rep_mobs_data %>%
      filter(pollutant == OBSERVED_POLLS[j]) %>%
      plot_time_series2()
  }
