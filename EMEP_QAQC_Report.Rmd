...
#show GB emissions
if (any(str_detect(emiss_domains, '^UK'))) {
  GB_emiss_tbl = emiss_list[str_detect(emiss_domains, '^UK')] %>%
    flatten_df() %>% 
    filter(Land == 'GB') %>% 
    select(-Land) %>% 
    mutate(pollutant = gt_var_labeller[pollutant],
           across(where(is.double), ~round(.x, 1))) %>%
    relocate(model, .before = inventory) %>% 
    gt() %>% 
    tab_header(title = md('**GB Emissions**')) %>% 
    cols_label(pollutant = 'Species',
               inventory = 'Inventory',
               model = 'Model',
               abs_diff = 'Difference (Gg)',
               rel_diff = 'Difference (%)') %>% 
    cols_align(align = 'right',
               columns = -pollutant) %>% 
    tab_options(data_row.padding = px(3),
...
Differences in emissions between the test run and EMEP inventory are saved in **`r emiss_list_tbl_pths[1]`** and plotted in **`r out_pths[1]`**. Similarly, differences in emissions between the test and reference runs are saved in **`r MBS_tbl_pths[1]`**, and plotted in **`r path(plots_pth_out, str_c(MBS_domains, '_', MBS_PLOT_FNAME))[1]`**, respectively.
...
Differences in emissions between the test run and EMEP inventory are saved in **`r emiss_list_tbl_pths[2]`** and plotted in **`r out_pths[2]`**. Similarly, differences in emissions between the test and reference runs are saved in **`r MBS_tbl_pths[2]`**, and plotted in **`r path(plots_pth_out, str_c(MBS_domains, '_', MBS_PLOT_FNAME))[2]`**, respectively..
...
Observations have been taken from the `r network_string1`. A list of used sites and their meta data is saved in **`r path(data_pth_out, '00Sites_used.rds')`**.
...
report_mobs_list = mobs_list[MOBS_STATION_REPORT] 
