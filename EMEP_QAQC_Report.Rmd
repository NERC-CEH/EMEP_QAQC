...
test_run_info = c(TEST_INNER_DIR, TEST_OUTER_DIR) %>% 
  path_common()

ref_run_info = c(REF_INNER_DIR, REF_OUTER_DIR) %>% 
    path_common()

if (is.null(TEST_INNER_DIR)) {
  inner_domain = NULL
} else {
  inner_domain = map_chr(path_split(TEST_INNER_DIR), last)
}

if (is.null(TEST_OUTER_DIR)) {
  outer_domain = NULL
} else {
  outer_domain = map_chr(path_split(TEST_OUTER_DIR), last)
}

EMEP_domains = list(outer_domain, inner_domain)

log_info('Test run directory: {test_run_info}')
log_info('Reference run directory: {ref_run_info}')
if (is.null(outer_domain)) {
  log_warn('Outer domain not submitted for this QAQC')
} else {
  log_info('Outer domain: {outer_domain}')
}

if (is.null(inner_domain)) {
...
``` {asis, echo = !is.null(outer_domain), eval = !is.null(outer_domain)}
**Outer Domain:** `r outer_domain`
```
``` {asis, echo = is.null(outer_domain), eval = is.null(outer_domain)}
**Outer Domain:** Not submitted for this QA/QC
```
``` {asis, echo = !is.null(inner_domain), eval = !is.null(inner_domain)}
...
``` {asis, echo = all(!map_lgl(list(outer_domain, inner_domain), is_null)) && COMPARE_FILE_SIZE, eval = all(!map_lgl(list(outer_domain, inner_domain), is_null)) && COMPARE_FILE_SIZE}
## `r outer_domain` Domain
```
``` {r, eval = !is.null(outer_domain) && COMPARE_FILE_SIZE, results = 'asis'}
fsize_tables[[1]]
```
``` {asis, echo = all(!map_lgl(list(outer_domain, inner_domain), is_null)) && COMPARE_FILE_SIZE, eval = all(!map_lgl(list(outer_domain, inner_domain), is_null)) && COMPARE_FILE_SIZE}
## `r inner_domain` Domain
```
``` {r, eval = !is.null(inner_domain) && COMPARE_FILE_SIZE, results = 'asis'}
fsize_tables[[2]]
```
...
if (!is.null(EMISSION_INVENTORY_PTH)) {
  ...
  emiss_list0 = map(list(TEST_OUTER_DIR, TEST_INNER_DIR), safely(compare_inv_mod_emissions), emiss_inv_pth = EMISSION_INVENTORY_PTH) %>%
    transpose()
  ...
  if (length(emiss_list) > 0) {
    ...
    page_titles = str_c(list(TEST_OUTER_DIR, TEST_INNER_DIR)[emiss_is_ok], '\n\n')
  ...
  emiss_exceedance_tbls = emiss_list0$result %>%
    map(safely(filter), abs(rel_diff) > EMISS_DIFF_THRESHOLD) %>% 
    transpose() %>%
    .$result %>% 
    map(check_nrow)
...
MBS_list0 = map2(list(TEST_OUTER_DIR, TEST_INNER_DIR), 
                list(REF_OUTER_DIR, REF_INNER_DIR), 
...
if (length(MBS_list) > 0) {
  ...
  walk2(MBS_plots, path(plots_pth_out, str_c(compact(EMEP_domains), '_', MBS_PLOT_FNAME)),
...
```
``` {asis, echo = all(!map_lgl(list(outer_domain, inner_domain), is_null)), eval = COMPARE_EMISSIONS}
## `r outer_domain` Domain
```

``` {asis, echo = COMPARE_EMISSIONS && !is.null(outer_domain) && is.null(emiss_exceedance_tbls[[1]] ) && !is.null(EMISSION_INVENTORY_PTH) && length(emiss_list) != 0 , eval = COMPARE_EMISSIONS && !is.null(outer_domain) && is.null(emiss_exceedance_tbls[[1]] ) && !is.null(EMISSION_INVENTORY_PTH)}
All test run emissions are within `r EMISS_DIFF_THRESHOLD`% of those in the supplied emission inventory file.
```

``` {asis, echo = COMPARE_EMISSIONS && !is.null(outer_domain) && !is.null(emiss_exceedance_tbls[[1]]) && !is.null(EMISSION_INVENTORY_PTH), eval = COMPARE_EMISSIONS && !is.null(outer_domain) && !is.null(emiss_exceedance_tbls[[1]]) && !is.null(EMISSION_INVENTORY_PTH)}
...
``` {r, eval = COMPARE_EMISSIONS && !is.null(outer_domain) && !is.null(emiss_exceedance_tbls[[1]]) && !is.null(EMISSION_INVENTORY_PTH)}
...
```
``` {asis, echo = COMPARE_EMISSIONS && !is.null(outer_domain) && is.null(MBS_exceedance_tbls[[1]] ) && length(MBS_list) != 0, eval = COMPARE_EMISSIONS && !is.null(outer_domain) && is.null(MBS_exceedance_tbls[[1]] ) && length(MBS_list) != 0}
...
``` {r, eval = COMPARE_EMISSIONS && !is.null(outer_domain) && !is.null(MBS_exceedance_tbls[[1]]) && length(MBS_list) != 0}
...
``` {asis, echo = COMPARE_EMISSIONS && !is.null(inner_domain) && is.null(emiss_exceedance_tbls[[2]]) && !is.null(EMISSION_INVENTORY_PTH) && length(emiss_list) != 0, eval = COMPARE_EMISSIONS && !is.null(inner_domain) && is.null(emiss_exceedance_tbls[[2]]) && !is.null(EMISSION_INVENTORY_PTH)}
All test run emissions are within `r EMISS_DIFF_THRESHOLD`% of those in the supplied emission inventory file.
```

``` {asis, echo = COMPARE_EMISSIONS && !is.null(inner_domain) && !is.null(emiss_exceedance_tbls[[2]]) && !is.null(EMISSION_INVENTORY_PTH), eval = COMPARE_EMISSIONS && !is.null(inner_domain) && !is.null(emiss_exceedance_tbls[[2]]) && !is.null(EMISSION_INVENTORY_PTH)}
Emissions of the following species in the test run differ by more than `r EMISS_DIFF_THRESHOLD`% from those in the supplied emission inventory file:
```

``` {r, eval = COMPARE_EMISSIONS && !is.null(inner_domain) && !is.null(emiss_exceedance_tbls[[2]]) && !is.null(EMISSION_INVENTORY_PTH)}
...
``` {asis, echo = COMPARE_EMISSIONS && !is.null(inner_domain) && is.null(MBS_exceedance_tbls[[2]] ) && length(MBS_list) != 0, eval = COMPARE_EMISSIONS && !is.null(inner_domain) && is.null(MBS_exceedance_tbls[[2]] ) && length(MBS_list) != 0}
All test run emissions are within `r EMISS_DIFF_THRESHOLD`% of those in the reference run.
```
...
``` {r, eval = COMPARE_EMISSIONS && !is.null(inner_domain) && !is.null(MBS_exceedance_tbls[[2]]) && length(MBS_list) != 0}
...
emep_stars_list = map2(EMEP_BUDGET_FNAME, EMEP_BUDGET_CRS, ~read_emep(.x, .y, var = emep_budget_vars, proxy = F)) %>% 
  set_names(nm = EMEP_BUDGET_FNAME)
if (is.null(BUDGET_MASK_FNAME)) {
  area_mask_list = list(full_domain = NULL)
} else {
  area_mask_list = map(BUDGET_MASK_FNAME, st_read) %>% 
    set_names(BUDGET_MASK_FNAME)
  log_info('Budget mask path: {path_abs(BUDGET_MASK_FNAME)}')
}

budget_iter_df = expand_grid(emep_stars_list, area_mask_list)

#get test file modelled year to put into output filename
mod_test_year = EMEP_BUDGET_FNAME[1] %>%
  read_stars(sub = names(emep_stars_list[[1]])[1]) %>%
  st_get_dimension_values('time') %>% 
  year()

budget_list = pmap(list(emep_stars = budget_iter_df$emep_stars_list, area_mask_sf = budget_iter_df$area_mask_list,
                      emep_run_name = names(budget_iter_df$emep_stars_list), area_name = names(budget_iter_df$area_mask_list)),
                 collate_budget, var_params_list = VAR_PARAMS_LIST) 
...
if (length(EMEP_BUDGET_FNAME) == 2) {
  ...
  budget_exceed_tbl = budget_df2 %>% 
    filter(abs(rel_diff) > BUDGET_DIFF_THRESHOLD)
