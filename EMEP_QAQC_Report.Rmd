...
missing_mobs = setdiff(names(report_mobs_list), MOBS_STATION_REPORT_TSERIES)
if (length(missing_mobs) == 0) {
  
} else if (length(missing_mobs) == 0) {
  cat(paste0('\nData for site ', missing_mobs, ' not collated.' ))
} else {
  cat(paste0('\nData for sites ', str_replace(str_c(missing_mobs, collapse = ', '), '(.*),', '\\1 and'), ' not collated.'))
}
