...
Test Directory: `r test_run_info`
\
Ref Directory: `r ref_run_info`
...
if (is.na(TEST_OUTER_DIR)) {
  ...
  fsize_df = compare_file_size(TEST_OUTER_DIR, REF_OUTER_DIR) %>%
...
if(is.na(TEST_INNER_DIR)) {
  ...
  fsize_df = compare_file_size(TEST_INNER_DIR, REF_INNER_DIR) %>%
  format_file_size_tbl() %>% 
  tab_options(data_row.padding = px(3),
              #table.font.size = pct(95),
              table.align='left')
...
MBS_list = map2(c(TEST_OUTER_DIR, TEST_INNER_DIR), 
...
emiss_list = map(c(TEST_OUTER_DIR, TEST_INNER_DIR), compare_inv_mod_emissions, webdabEMEP_pth = WEBDABEMEP_PTH)
...
page_titles = str_c(c(TEST_OUTER_DIR, TEST_INNER_DIR), '\n\n')
...
emiss_list_tbl_pths = str_c(EMEP_domains, INV_MOD_EMISS_TABLE_FNAME, sep = '_')
MBS_tbl_pths = str_c(EMEP_domains, MBS_TABLE_FNAME, sep = '_')
...
EMEP_BUDGET_FNAME = map_chr(EMEP_BUDGET_DIR, ~dir_ls(.) %>% str_subset('fullrun'))
...
if (PLOT_COMPARISON_MAPS == T) {
  ...
  comp_map_data = future_map(comp_map_vars[8], ~calculate_emep_diff(var = .x,
  ...
  report_map_data = future_map(report_map_vars, ~calculate_emep_diff(var = .x,
                                                                     run_labels = RUN_LABELS,
                                                                     outer_test_fname = select_file_from_dir(TEST_OUTER_DIR),
                                                                     outer_ref_fname = select_file_from_dir(REF_OUTER_DIR),
                                                                     inner_test_fname = select_file_from_dir(TEST_INNER_DIR),
  ...
  #plot inner domain only if inner domain files are supplied
  if (all(!is.na(c(TEST_INNER_DIR, REF_INNER_DIR)))) {
    report_map_data = future_map(report_map_vars, ~calculate_emep_diff(var = .x,
                                                                       run_labels = RUN_LABELS,
                                                                       outer_test_fname = select_file_from_dir(TEST_OUTER_DIR),
                                                                       outer_ref_fname = select_file_from_dir(REF_OUTER_DIR),
