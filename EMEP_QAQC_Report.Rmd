...
comp_map_data = future_map(comp_map_vars, ~calculate_emep_diff(var = .x,
                                                                  ...
                                                                  test_crs = TEST_CRS, ref_crs = REF_CRS ))
...
mobs_wide = mobs_list %>%
  future_map(~pivot_wider(.x, id_cols = c(date, code), names_from = c(pollutant), values_from = c(obs, mod)))
...
```{r mobs_report_base_map, include=FALSE, eval = PLOT_MOBS}
#for report:
...
#if the inner domain is UK plot trunk roads for better orientation - WIP
...
ref_mobs_pths = dir_ls(REF_MOBS_DIR, regexp = '\\.rds') %>%
  str_subset('(S|s)ites', negate = T)
ref_mobs_list = future_map(ref_mobs_pths, read_rds) %>%
  future_map(mobs_to_long) %>%
  #temporary to ensure code column is never NA due to a previous bug
  future_map(~mutate(.x, code = if_else(is.na(code), na.omit(unique(.$code)), code)))
...
``` {r, echo = FALSE, eval = PLOT_MOBS, fig.width = 10, fig.height = 10*(domain_dims[2]/domain_dims[1]*1.1)}
sites_geo = sites_geo %>%
...
```{r, echo = F, eval = PLOT_MOBS && nrow(sites_geo_map_sub) != 0 && MOBS_STATION_REPORT_MAP == T, fig.width = 10, fig.height = 10*(domain_dims[2]/domain_dims[1]*1.1), results = 'asis'}
