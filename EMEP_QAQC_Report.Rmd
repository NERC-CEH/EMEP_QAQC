...
```{r Inventory_check_report, eval = COMPARE_EMISSIONS && !is.na(EMISSION_INVENTORY_PTH)}
...
``` {asis, echo = COMPARE_EMISSIONS && !is.na(outer_domain) && is.null(emiss_exceedance_tbls[[1]] ) && !is.na(EMISSION_INVENTORY_PTH), eval = COMPARE_EMISSIONS}
...
```
``` {asis, echo = COMPARE_EMISSIONS && !is.na(outer_domain) && is.null(MBS_exceedance_tbls[[1]] ) && length(MBS_list) != 0, eval = COMPARE_EMISSIONS}
...
``` {asis, echo = COMPARE_EMISSIONS && !is.na(inner_domain) && is.null(emiss_exceedance_tbls[[2]]) && !is.na(EMISSION_INVENTORY_PTH), eval = COMPARE_EMISSIONS}
All test run emissions are within `r EMISS_DIFF_THRESHOLD`% of those in the supplied emission inventory file.
```
``` {asis, echo = COMPARE_EMISSIONS && !is.na(inner_domain) && !is.null(emiss_exceedance_tbls[[2]]) && !is.na(EMISSION_INVENTORY_PTH), eval = COMPARE_EMISSIONS}
Emissions of the following species in the test run differ by more than `r EMISS_DIFF_THRESHOLD`% from those in the supplied emission inventory file:
...
``` {asis, echo = COMPARE_EMISSIONS && !is.na(inner_domain) && is.null(MBS_exceedance_tbls[[2]] ) && length(MBS_list) != 0, eval = COMPARE_EMISSIONS}
All test run emissions are within `r EMISS_DIFF_THRESHOLD`% of those in the reference run.
```
``` {asis, echo = COMPARE_EMISSIONS && !is.na(inner_domain) && !is.null(MBS_exceedance_tbls[[2]]) && length(MBS_list) != 0, eval = COMPARE_EMISSIONS}
Emissions of the following species in the test run differ by more than `r EMISS_DIFF_THRESHOLD`% from those in the reference run:
`r MBS_exceedance_tbls[[2]]`
```
...
sites_used_files = dir_ls(data_pth_out, regexp = '(M|m)ites.*\\.rds')
