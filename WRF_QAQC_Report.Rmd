...
#write out sites used in the qaqc
...
names(wrf_data_list) = wrf_data_list %>% 
  map_chr(~pull(.x, code) %>% unique)

#download observation data safely
noaa_tlist = future_map(names(wrf_data_list), safely(importNOAA), year = yr) %>% 
  transpose()

is_ok <- noaa_tlist$error %>%
  map_lgl(is_null)

noaa_list = noaa_tlist$result[is_ok] %>% 
  compact() %>% 
  future_map(format_NOAA) %>% 
  future_map(~mutate(.x, scenario = 'obs'))

names(noaa_list) = noaa_list %>% 
  map_chr(~pull(.x, code) %>% unique() %>% as.character())

wrf_data_list2 = wrf_data_list[names(noaa_list)]

mobs_list = reduce(c(wrf_data_list2, noaa_list), bind_rows) %>% 
  group_split(code) %>%
...
names(mobs_list) = mobs_list %>% 
  map_chr(~pull(.x, code) %>% unique)
rm(wrf_data_list, wrf_data_tlist, wrf_data_list2, noaa_list, noaa_tlist)
